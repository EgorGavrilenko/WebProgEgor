...
var camRollFunction = (function() {
    ...
    function validatePhotoPost(photoPost) {
        ...
        if (!(typeof photoPost['photoLink'] === "string"))
            return false;
        if (!(is_array(photoPost['tag'])))
            return false;
        if (!(is_array(photoPost['like'])))
            return false;
        for (let i = 0; i < photoPost['tag'].length; i++)
            if (!(typeof photoPost['tag'][i] === "string"))
                return false;
        for (let i = 0; i < photoPost['like'].length; i++)
            if (!(typeof photoPost['like'][i] === "string"))
                return false;
        return true;
    };
    ...
    function editPhotoPost(index, change) {
        ...
        if (k === undefined) {
            ...
            if ('tag' in change) {
                if (!(photoPost['tag'].isArray)) {
                    let t = true;
                    for (let i = 0; i < change['tag'].length; i++)
                        if (!(typeof change['tag'][i] === "string"))
                            t = false;
                    if (t)
            ...
            if ('like' in change) {
                if (!(change['like'].isArray)) {
                    ...
                    for (let i = 0; i < change['like'].length; i++)
                        if (!(typeof change['like'][i] === "string"))
                            t = false;
                    if (t)
