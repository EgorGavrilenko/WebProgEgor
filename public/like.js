...
var smile = function() {
    ...
    function like() {
        ...
        events.getPost(id).then((result) => {
            
            likeFunction(JSON.parse(result,function(key, value) {
    ...
    function likeFunction(post) {
        ...
        if (a == -1) {
            ...
            events.addLike(post.id,user).catch((err) =>
            console.log(err.message));
        } else {
            elem.className = "like_no";
            events.removeLike(post.id,user).catch((err) =>
            console.log(err.message));
        }
    }
    return {
